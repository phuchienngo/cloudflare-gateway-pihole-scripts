...
const request = async (url, options) => {
  ...
  while (retryCount < 10) {
    try {
      ...
      lastException = e
      console.error("Error while processing, prepare for retrying", e)
      const delayTime = 100 * Math.pow(2, retryCount++);
      await delay(delayTime);
    }
  }

  console.error("Reach max attempt, stopping", lastException)
  throw lastException
};
